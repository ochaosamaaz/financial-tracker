...
class DuitTracker {
    ...
    setTheme(theme) {
        ...
        const darkThemes = ['dark','midnight','charcoal'];
        if (icon) icon.className = darkThemes.includes(theme)?'fas fa-sun':'fas fa-moon';
    ...
    toggleTheme() {
        ...
        const darkThemes = ['dark','midnight','charcoal'];
        this.setTheme(darkThemes.includes(c)?'light':'dark');
    }
    ...
    bindEvents() {
        ...
        // Theme picker
        document.querySelectorAll('.theme-option').forEach(opt => {
            opt.addEventListener('click', () => this.setTheme(opt.dataset.theme));
        });
        // Copy code button
